...
```tsx
import { QeetLogo, QeetLogoOnLight, QeetLogoOnDark } from "@qeetix/brand";
...
`variant="auto"` (default) swaps via the `.dark` class strategy + the Qeetix Tailwind
`dark:` variant, so it Just Works when you import `@qeetix/ui/styles.css`. Without those
...
Raw SVGs are also shipped: `@qeetix/brand/assets/qeet-logo-on-light.svg`, `.../qeet-logo-on-dark.svg`.

## Icons

```tsx
import { IconPasskey, IconMfaShield } from "@qeetix/brand";
...
> only the artwork will change. Logos regenerate from `assets/raw` via `pnpm --filter @qeetix/brand generate`.
