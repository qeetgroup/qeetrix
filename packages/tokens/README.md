...
The single source of truth for Qeetix design tokens. Authored as **W3C Design Tokens**
...
| --- | --- | --- |
| `qeetix.css` | `@qeetix/tokens/qeetix.css` | **shadcn / Base-UI bridge** — `:root` + `.dark` with the *unprefixed* runtime vars (`--background`, `--primary`, `--radius`, `--sidebar-*`, `--chart-*`). `@qeetix/ui` imports this. Value-for-value identical to the original Qeet ID `index.css`. |
| `qeetix.tokens.css` | `@qeetix/tokens/tokens.css` | Full raw token export, **`--qx-`** prefixed (`--qx-color-text-primary`, `--qx-space-l`, `--qx-radii-base`, …). For advanced / white-label consumers. |
| `qeetix.tokens.json` | `@qeetix/tokens/tokens.json` | Resolved tokens per theme `{ light, dark }`. Cross-platform (marketing, future Flutter SDK). |
...
```bash
pnpm --filter @qeetix/tokens build      # regenerate dist/
pnpm --filter @qeetix/tokens validate   # WCAG-AA contrast check (blocks on body-text regressions)
```
